...
async function seed() {
	...
	// 3. Service Images (Pinata IPFS Uploads)
	for (const item of serviceSeed) {
		if (item.photos && item.photos.length > 0) {
			const photoUrl = item.photos[0];
			const ipfsHash = photoUrl.replace("https://gateway.pinata.cloud/ipfs/", "");
			await ServiceImage.findOneAndUpdate(
				{ serviceId: item._id, ipfsHash },
				{
					serviceId: item._id,
					ipfsHash,
					url: photoUrl,
	...
	console.log("All 13 Service Images synced!");
